...
export default async function runExecutor(options: BuildExecutorSchema, context: ExecutorContext) {
  ...
  const nodeModulesPath = path.resolve(context.root, 'node_modules')

  const appPath = path.resolve(context.root, context.projectsConfigurations.projects[context.projectName].root)
